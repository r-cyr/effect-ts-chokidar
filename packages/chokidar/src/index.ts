              ...
              H.publish_(
                ...
                Ex.fail(
                  new FileWatcherError(
                    "Error occured while watch path ${}",
                    O.some(error)
                  )
                )
              )
            ...
            switch (eventName) {
              case "add":
                T.run(
                  H.publish_(
                    this.fsEventsHub,
                    Ex.succeed(new FileAdded(path, O.fromNullable(stats)))
                  )
                )
                break
              case "unlink":
                T.run(
                  H.publish_(
                    this.fsEventsHub,
                    Ex.succeed(new FileRemoved(path, O.fromNullable(stats)))
                  )
                )
                break
              case "change":
                T.run(
                  H.publish_(
                    this.fsEventsHub,
                    Ex.succeed(new FileChanged(path, O.fromNullable(stats)))
                  )
                )
                break
              case "addDir":
                T.run(
                  H.publish_(
                    this.fsEventsHub,
                    Ex.succeed(new DirectoryAdded(path, O.fromNullable(stats)))
                  )
                )
                break
              case "unlinkDir":
                T.run(
                  H.publish_(
                    this.fsEventsHub,
                    Ex.succeed(new DirectoryRemoved(path, O.fromNullable(stats)))
                  )
                )
                break
            }
